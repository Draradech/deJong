...
import { WebGPU, DownloadInfo } from './webgpu.js';
...
async function main() {
  ...
  webgpu.addComputePass(
    shader,
    'pass2',
    1,
    new Map().set('uniform', 0).set('timestamp', 1).set('frameinfo', 2).set('indirect', 3)
  );
  ...
  let gtime: number, npoints: number;
